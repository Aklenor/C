...
void add_next(int value, struct Item* item){
    if (item == NULL){
        printf("Item points to NULL\n");
        return;
    }
...
static void add_prev(int* value, struct Item* item){
    if (item == NULL){
        printf("Item points to NULL\n");
        return;
    }
...
static struct Item* insert_next(int* value, struct Item* item){
    if (item == NULL){
        printf("Item points to NULL\n");
        return NULL;
    }
...
void LLadd(int value, struct ListHeader* header){
    if (header == NULL){
        printf("LLadd(): *ListHeader poitns to NULL\n");
        return;
    }
    ...
    if (header->firstItem == NULL && header->lastItem == NULL){
        ...
        header->length = 1;
        header->firstItem->value = int_dup(value);
        return;
    }else {
